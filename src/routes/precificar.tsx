...
import { Plus, Trash2, Save, Printer, Sparkles, AlertCircle, CheckCircle2, XCircle } from "lucide-react";
import { z } from "zod";
...
function Pricing() {
  ...
  const result = useMemo(() => calculate({
    minutes, labCost, otherDirect, supplies: usages, customCosts, suppliesIndex, fixedPerMin: fpm, settings,
  }), [minutes, labCost, otherDirect, usages, customCosts, suppliesIndex, fpm, settings]);
          ...
          <Card>
            ...
            <div className="grid sm:grid-cols-2 gap-3 mt-3">
              <div>
                ...
                <Input type="number" step="0.01" value={labCost} onChange={e => setLabCost(+e.target.value)} />
              </div>
              <div>
          ...
          <div className="card-premium p-5 relative overflow-hidden">
            ...
            <div className="mt-4 space-y-3">
              <div className="rounded-xl bg-gold-gradient p-4 text-white">
                <div className="text-[11px] uppercase tracking-wider opacity-90">Preço recomendado · Cartão</div>
                <div className="text-3xl font-display font-extrabold mt-1">{BRL(result.priceCard)}</div>
                <div className="text-xs opacity-90 mt-1">Margem líquida {result.netMargin.toFixed(1)}%</div>
              </div>
              <div className="rounded-xl bg-secondary p-4">
                <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Preço recomendado · PIX</div>
                <div className="text-2xl font-display font-bold mt-1">{BRL(result.pricePix)}</div>
              </div>
            </div>
            ...
            <dl className="mt-5 space-y-2 text-sm">
              {[
                ...
                ["• Laboratório", BRL(labCost)],
                ["• Outros diretos", BRL(otherDirect)],
